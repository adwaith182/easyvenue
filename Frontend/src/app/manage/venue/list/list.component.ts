...
@Component({
  ...
  `],
    providers: [VenueService, ConfirmationService, TitleCasePipe, MessageService]
})
export class ListComponent implements OnInit {
    ...
    getCategory() {
        ...
        if (roomobj.id) {
            var query = "?filterByDisable=false&filterByStatus=true&filterByParent=" + roomobj.id + "&sortBy=name&orderBy=ASC";
            this.categoryService.getCategoryList(query).subscribe(
                data => {
                    var roomlist = data.data.items;
                    roomlist.forEach(element => {
                        this.roomtypeList.push({ id: element.id, slug: element.slug, name: element.name });
                    })
                    this.roomtypeList.map((x, i) => {
                        this.venueroomForm.addControl(x.slug + "room", this.fb.control('0'));
                        this.venueroomForm.addControl(x.slug + "price", this.fb.control('0'));
                    })
                },
                err => {
        ...
        let foodmenuTypeobj = parentlist.find(o => o.slug === "foodmenutypes");
        if (foodmenuTypeobj.id) {
            var query = "?filterByDisable=false&filterByStatus=true&filterByParent=" + foodmenuTypeobj.id + "&sortBy=name&orderBy=ASC";
            this.categoryService.getCategoryList(query).subscribe(
                data => {
                    var foodmenuttypelist = data.data.items;
                    foodmenuttypelist.forEach(element => {
                        this.foodmenutypeList.push({ id: element.id, slug: element.slug, name: element.name });
                    })
    ...
    addRoom(venue) {
        ...
        this.roomtypeList.forEach(roomData => {
            ...
            this.venueroomForm.controls[roomData.slug + 'room'].setValue(obj['room']);
            this.venueroomForm.controls[roomData.slug + 'price'].setValue(obj['price']);
        })
    ...
    onRoomSubmit() {
        ...
        var venueRoomData = [];
        this.roomtypeList.forEach(roomData => {
            venueRoomData.push({ 'id': roomData.id, 'slug': roomData.slug, 'room': venueroomData[roomData.slug + 'room'], 'price': venueroomData[roomData.slug + 'price'] });
        })
        this.VenueService.updateVenue(this.selectedVenue, { 'roomData': venueRoomData }).subscribe(res => {
            this.roomMangement = false;
            this.refreshVenueList(this.lastTableLazyLoadEvent);
            this.messageService.add({ key: 'toastmsg', severity: 'success', summary: 'Successful', detail: 'Venue Room data saved', life: 6000 });
            setTimeout(() => {
                this.router.navigate(['/manage/venue']);
            }, 2000);
        }, ((err) => {
            this.messageService.add({ key: 'toastmsg', severity: 'error', summary: err.error.error, detail: 'Update Venue Room failed', life: 6000 });
        }));
    }
    addMenu(venue) {
        this.roomMangement = false;
        ...
        this.foodmenutypeList.map((menu, k) => {
            this.foodtypeList.map((x, i) => {
                this.menutypeForm.addControl(x.slug + menu.slug + "price", this.fb.control('0'));
                if (venue.foodMenuType[x.slug] != undefined) {
                    let obj = venue.foodMenuType[x.slug].find(o => o.slug === menu.slug);
                    this.menutypeForm.controls[x.slug + menu.slug + "price"].setValue(obj['value']);
                }
            });
        });
        this.venueName = venue.name;
    ...
    onMenutypeSubmit() {
        ...
        this.foodtypeList.map((x, i) => {
            var menupriceArr = [];
            this.foodmenutypeList.map((menu, k) => {
                var obj = { 'slug': menu.slug, value: this.menutypeForm.get(x.slug + menu.slug + "price").value };
        ...
        this.VenueService.updateVenue(this.selectedVenue, { 'foodMenuType': menuData }).subscribe(res => {
            this.menutypeMangement = false;
            this.refreshVenueList(this.lastTableLazyLoadEvent);
            this.messageService.add({ key: 'toastmsg', severity: 'success', summary: 'Successful', detail: 'Venue Food menu saved', life: 6000 });
            setTimeout(() => {
                this.router.navigate(['/manage/venue']);
            }, 2000);
        }, ((err) => {
            this.messageService.add({ key: 'toastmsg', severity: 'error', summary: err.error.error, detail: 'Update Venue Food menu failed', life: 6000 });
        }));
    }
    exportExcel() {
        this.downloadFlg = true;
        this.refreshVenueList(this.lastTableLazyLoadEvent);
        var propertiesRemove = ['_id', 'created_by', 'created_at', 'updated_at', '__v', 'updated_by', 'createduserdata', 'updateduserdata'];
    ...
    changeStatus(venue) {
        this.confirmationService.confirm({
            message: 'Are you sure you want to change the status of  ' + (this.titlecasePipe.transform(venue.name)) + '?',
            header: 'Confirm',
            ...
            accept: () => {
                ...
                this.VenueService.updateVenue(venue.id, updateData).subscribe(
                    data => {
                        this.messageService.add({ key: 'toastmsg', severity: 'success', summary: 'Successful', detail: 'Venue Status Updated', life: 3000 });
                    },
                    err => {
                        this.errorMessage = err.error.message;
                    }
    ...
    changeAssured(venue) {
        this.confirmationService.confirm({
            message: 'Are you sure you want to mark ' + (this.titlecasePipe.transform(venue.name)) + ' as Assured?',
            header: 'Confirm',
            icon: 'pi pi-exclamation-triangle',
            accept: () => {
                ...
                this.VenueService.updateVenue(venue.id, updateData).subscribe(
                    data => {
                        this.messageService.add({ key: 'toastmsg', severity: 'success', summary: 'Successful', detail: 'Venue marked as Assured', life: 3000 });
                    },
                    err => {
                        this.errorMessage = err.error.message;
                    }
    ...
    onDelete(venue) {
        this.confirmationService.confirm({
            ...
            accept: () => {
                ...
                this.VenueService.updateVenue(venue.id, updateData).subscribe(
                    data => {
                        this.messageService.add({ key: 'toastmsg', severity: 'success', summary: 'Successful', detail: 'Venue Deleted', life: 3000 });
                        this.refreshVenueList(this.lastTableLazyLoadEvent);
                    },
                    err => {
                        this.errorMessage = err.error.message;
    ...
    addSlotAvailability(venue) {
        this.router.navigateByUrl('/manage/venue/slot-availability/' + venue.id);
    }
    /**
